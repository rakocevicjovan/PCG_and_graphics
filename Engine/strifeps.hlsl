...
float4 raymarch(float3 ro, float3 rd)
{
    ...
    for (int i = 0; i < NUM_STEPS; ++i)
    {
        ...
        density += mask * adjStepSize;
        //density += mask * phase; //step (sampled r, .2) for cartoony, blocky mask
        //density += adjStepSize * mask * phase;
    ...
    //if (IntersectRaySphere(rayOrigin, rayDir, float3(0., -earthRadius, 0.), earthRadius + 10.f) > 0.) return float4(1., 0., 0., 1.);
