...
Perlin::Perlin()
{
	// Populating with "random" permutation - but the seed is fixed for now, easier to test
...
{
	return static_cast<float>(pow(t, 3) * (t * (6 * t - 15) + 10));
}
...
float Perlin::perlin2d(SVec2 pos)
{
	...
	SVec2 bottomLeft(	iLeft + 0.f, iBottom + 0.f);
	SVec2 bottomRight(iLeft + 1.f, iBottom + 0.f);
	SVec2 topLeft(		iLeft + 0.f, iBottom + 1.f);
	SVec2 topRight(		iLeft + 1.f, iBottom + 1.f);
