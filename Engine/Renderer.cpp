...
bool Renderer::Initialize(int windowWidth, int windowHeight, HWND hwnd, InputManager& inMan){
	...
	Camera cam(cMat.Invert(), projectionMatrix);
...
bool Renderer::RenderFrame(const std::vector<Model*>& models, const Camera& cam){
	...
	_shaders[0].SetShaderParameters(_deviceContext, *(models[0]), cam.GetViewMatrix(), cam.GetProjectionMatrix(), _lights[0], cam.GetCameraMatrix().Translation(), 0.016f);
	...
	//shaderDepth.SetShaderParameters(_deviceContext, *_models[0], cam.GetViewMatrix(), cam.GetProjectionMatrix());
