...
public:
	...
	void createDepthStencil(ID3D11Device* device, UINT w, UINT h, DXGI_FORMAT format = DXGI_FORMAT_D24_UNORM_S8_UINT, UINT extraFlags = 0u, UINT arrSize = 1u)
	{
		...
		if (FAILED(device->CreateDepthStencilView(_dsb.Get(), &depthStencilViewDesc, &_debugDsv)))
		{
		...
		if (extraFlags && D3D11_BIND_SHADER_RESOURCE)
		{
			...
			if (FAILED(device->CreateShaderResourceView(_dsb.Get(), &srvDesc, &_srv)))
			{
...
private:
	...
	Microsoft::WRL::ComPtr<ID3D11DepthStencilView> _debugDsv{ nullptr };
